...
namespace OpenApoc
{
		...
		gles_initialized_(false)
	{
		...
		if (!eglInitialized)
		{
			LogError("Could not initialize EGL, number of failed functions: %d", eglInitialized.GetNumMissing());
		}
		auto glInitialized = gl::sys::LoadFunctions();
		if (!glInitialized)
		{
			LogError("Could not initialize GLES2, number of failed functions: %d", glInitialized.GetNumMissing());
		}
	}
	...
	bool GLContext::InitEGLSurface()
	{
		...
		*/
		const EGLint attribs[] = {
			egl::conf::RENDERABLE_TYPE, egl::conf::OPENGL_ES2_BIT, //Request opengl ES2.0
			egl::conf::SURFACE_TYPE, egl::conf::WINDOW_BIT,
			egl::conf::BLUE_SIZE, 8,
			egl::conf::GREEN_SIZE, 8,
			egl::conf::RED_SIZE, 8, 
			egl::conf::DEPTH_SIZE, egl::DONT_CARE,
			egl::conf::STENCIL_SIZE, egl::DONT_CARE,
			egl::NONE };
		color_size_ = 8;
		depth_size_ = 16;
